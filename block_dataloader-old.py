...
def generate_dataloader_block(raw_graph, full_block_dataloader, args):
	...
	if args.num_batch == 1:
		if args.num_re_partition == 0:
			return full_block_dataloader,[1], [0, 0, 0]
		if args.num_re_partition == 1:
			return m_split_test(raw_graph, full_block_dataloader, args,)
...
def generate_dataloader_gp_block(raw_graph, full_block_dataloader, args):
	...
	b_gen_t_mean = 0
	gp_time = 0
	before_len = len(data_loader)
	for _ in range(args.num_re_partition):
		data_loader_re, weights_list, gp_time, [check_t,b_gen_t, b_gen_t_mean] = re_partition_block(raw_graph, full_blocks, args,  data_loader, weights_list)
	print('----------===============-------------===============-------------the number of batches *****----', len(data_loader))
	print()
	print('before re, number of batches: ', before_len )
	print('after number of batches: ',len(data_loader_re))
	args.num_batch = len(data_loader_re) - args.num_re_partition   #reset  the original number of batches
	if check_t:
		connect_checking_time_list = connect_checking_time_list + [check_t]

	block_gen_time_total = block_gen_time_total + b_gen_t
	batch_blocks_gen_mean_time = block_gen_time_total/len(data_loader_re)/args.num_layers
	print('re graph partition time: ', gp_time)
	print()
	##########################################################################################################

	
	return data_loader_re, weights_list, [sum(connect_checking_time_list), block_gen_time_total, batch_blocks_gen_mean_time]


	##########################################################################################################
def m_split_test(raw_graph, full_block_dataloader, args,):
	data_loader = full_block_dataloader
	...
	src, dst, blocks = full_block_dataloader[0]
	Block = blocks[0]
	in_degrees = Block.in_degrees()
	# in_degrees = Block.in_degrees(109418)
	# print('batch ', batch_id)
	print('in degree ', in_degrees) # The i-th element is the in-degree of the i-th input node
	...
	print('----------===============-------------===============-------------the number of batches *****----', len(data_loader))
	print()
	fan_out_in_degree_node_indices = (in_degrees == processed_fan_out[0]).nonzero().squeeze()       # in-degree = 10
	#-----------------------------------------------------------------------------------------------test other in-degree
	# fan_out_in_degree_node_indices = (in_degrees == (processed_fan_out[0]-1)).nonzero().squeeze() # in-degree = 10-1=9
	# fan_out_in_degree_node_indices = (in_degrees == (processed_fan_out[0]-2)).nonzero().squeeze() # in-degree = 10-2=8
	# fan_out_in_degree_node_indices = (in_degrees == (processed_fan_out[0]-3)).nonzero().squeeze() # in-degree = 10-3=7
	# fan_out_in_degree_node_indices = (in_degrees == (processed_fan_out[0]-4)).nonzero().squeeze() # in-degree = 10-4=6
	# fan_out_in_degree_node_indices = (in_degrees == (processed_fan_out[0]-5)).nonzero().squeeze() # in-degree = 10-5=5
	# fan_out_in_degree_node_indices = (in_degrees == (processed_fan_out[0]-6)).nonzero().squeeze() # in-degree = 10-6=4
	# fan_out_in_degree_node_indices = (in_degrees == (processed_fan_out[0]-7)).nonzero().squeeze() # in-degree = 10-7=3
	# fan_out_in_degree_node_indices = (in_degrees == (processed_fan_out[0]-8)).nonzero().squeeze() # in-degree = 10-8=2
	# fan_out_in_degree_node_indices = (in_degrees == (processed_fan_out[0]-9)).nonzero().squeeze() # in-degree = 10-9=1


	print('fan out in degree nodes number : ',len(fan_out_in_degree_node_indices))
	print('drop half of them in-edges, instead of output nodes, as the ')
	print(' the number of edges now: ', Block.num_edges())
	print()
	node_ids_to_process = fan_out_in_degree_node_indices[:int(len(fan_out_in_degree_node_indices)/2)]
	in_edges_to_drop = Block.in_edges(node_ids_to_process, form='all')
	Block.remove_edges(in_edges_to_drop[2])
	in_degrees = Block.in_degrees()
	# in_degrees = Block.in_degrees(109418)
	# print('batch ', batch_id)
	print('after drop half nodes in edges, the left in degree ', in_degrees) # The i-th element is the in-degree of the i-th input node
	
	# print(torch.histc(in_degrees.float()))
	print(Counter(in_degrees.tolist()))
	print('after remove half edges the number of edges now: ', Block.num_edges())
	print()
 
	# print('remove these in degree == 0 output nodes: ')
	# zero_in_degree_node_indices = (in_degrees == 0).nonzero().squeeze()
	# Block = dgl.remove_nodes(Block,zero_in_degree_node_indices)	# not works well
	# in_degrees = Block.in_degrees()
	# # in_degrees = Block.in_degrees(109418)
	# # print('batch ', batch_id)
	# print('after drop zero in degree nodes, the left in degree ', in_degrees) # The i-th element is the in-degree of the i-th input node
	
	# # print(torch.histc(in_degrees.float()))
	# print(Counter(in_degrees.tolist()))

	data_loader = []
	data_loader.append((src, dst, [Block]))
	##########################################################################################################

	return data_loader, [1], [0, 0, 0]
...
def re_partition_block(raw_graph, full_blocks, args,  data_loader, weights_list):
	...
	# b_id = in_degree_gp_first_layer_output_standard(args,  data_loader) # -------- output in-degree
	########################
	...
	if type(b_id) == bool:  # if it loads balance, just skip re-partition step
		return data_loader, weights_list, 0,[0,0,0]
	# else 
	if args.re_partition_method ==" " :
		return data_loader, weights_list, 0,[0,0,0]

	# if args.re_partition_method =='m-split' and args.num_batch ==1:	# we only test the full batch for m-split ideal performance
	# 	from collections import Counter
	# 	src, dst, blocks = data_loader[0]
	# 	Block = blocks[0]
	# 	in_degrees = Block.in_degrees()
	# 	# in_degrees = Block.in_degrees(109418)
	# 	print('batch ', batch_id)
...
def in_degree_gp_first_layer_output_standard(args,  data_loader):
	...
	# if len_src_max > len_src_min * 1.1: # intuitive way to decide whether it need re partition or not
	# 	b_id = len_src_list.index(len_src_max)
	from collections import Counter
