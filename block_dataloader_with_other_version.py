...
def func(output_nid, current_layer_block, dict_nid_2_local ):
	...
	return (src_nid, output_nid, global_eid_tensor, local_output_nid)
...
def check_connections_block(batched_nodes_list, current_layer_block):
	...
	dict_nid_2_local = dict(zip(src_nid_list, range(len(src_nid_list))))
	...
	# for step, output_nid in enumerate(batched_nodes_list):
	# 	zip_args = list(zip(output_nid, current_layer_block, dict_nid_2_local))
	# 	tmp = pool.starmap(func, zip_args)
	# 	print(tmp)
	# 	res.append(tmp)
