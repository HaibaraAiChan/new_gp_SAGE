	...
	for run in range(args.num_runs):
		...
		for epoch in range(args.num_epochs):
			...
			if epoch >= args.log_indent:
				...
				pure_train_time = sum(data_loading_t) +sum(block_to_t) +sum(modeling_t)+sum(loss_cal_t)+sum(backward_t)+(ttend-tte)
				print('* Pure training time/epoch {}'.format(pure_train_time))
...
def main():
	...
	argparser.add_argument('--selection-method', type=str, default='shared_neighbor_graph_partition_s0')
	#-------------------------------------------------------------------------------------------------------
	# argparser.add_argument('--selection-method', type=str, default='Betty')
	...
	argparser.add_argument('--re-partition-method', type=str, default=' ')
	# argparser.add_argument('--re-partition-method', type=str, default='m-split')
	# argparser.add_argument('--re-partition-method', type=str, default='Betty')
	# argparser.add_argument('--selection-method', type=str, default='shared_neighbor_graph_partition_s1')
	argparser.add_argument('--num-re-partition', type=int, default=0) # 1 will enter m-split, 0 will not
	#-------------------------------------------------------------------------------------------------------
	...
	argparser.add_argument('--fan-out', type=str, default='8')
...
#---------------------------------------------------------------------------------------
	# argparser.add_argument('--num_batch', type=int, default=16)
	...
	# argparser.add_argument('--num_batch', type=int, default=64)
...
#--------------------------------------------------------------------------------------
	...
	print('the largest in-degree: ',torch.max(in_degrees))
	print('the smallest in-degree: ',torch.min(in_degrees))
	in_degrees= in_degrees.type(torch.DoubleTensor)
	print('the average in-degree: ',torch.mean(in_degrees))
	print()
	print('the average degree: ', g.batch_num_edges()/g.batch_num_nodes())
