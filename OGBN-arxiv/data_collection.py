...
def compute_efficiency(filename, args, full_input_size, first_layer_output_size, redundancy=0):
	...
	# with open(filename) as f:
		for line in f:
			...
			# if line.startswith("graph partition algorithm spend time"):
			# 	# print('*&--=--'*40)
			# 	gp_time.append(float(line.split()[-1]))

			if "selection method" in line:
				# print('*&--=--'*40)
				gp_time.append(float(line.split()[-1]))
	...
	if redundancy == 1:
		...
		res.update({'pure graph partition spend time': 0})
	else:
		res.update({'pure graph partition spend time': mean(gp_time)})
	if not OOM_flag:
...
def data_collection( file_in, model, path_2, args):
	if 'ogbn' in file_in:
	...
	save_ = args.save_path + 'logs/'+args.selection_method+'/'+str(args.num_layers)+'_layer_'+args.file+'_'+args.aggre+'_'+str(args.hidden)
...
if __name__=='__main__':
	...
	# argparser.add_argument('--selection-method', type=str, default='shared_neighbor_graph_partition_set_one')
	...
	data_collection(file_in, args.model, path_2, args)		
