...
def load_bucket_labels( bucket_output,batch_pred, blocks, labels, device ):
	...
	bucket_pred = batch_pred[idx].to(device)
	bucket_labels = labels[idx].to(device)
	...
	for run in range(args.num_runs):
		...
		for epoch in range(args.num_epochs):
			...
			for step, (input_nodes, seeds, blocks) in enumerate(full_batch_dataloader):
				...
				seeds = seeds.int().to(device)
				degrees = blocks[0].in_degrees(seeds)
				...
				for deg in unique_degrees:
					...
					batch_loss = loss_fcn(batch_pred, batch_labels)
					bucket_loss = batch_loss*(len(bucket_outputs)/len(seeds))

					print('ratio: ', len(bucket_outputs)/len(seeds))
					print('bucket_loss : ', bucket_loss)
					loss_sum.append(bucket_loss.cpu().detach()) 

					bucket_loss.backward()
				print('loss_sum : ', torch.sum(torch.stack(loss_sum)))
				optimizer.step()
				optimizer.zero_grad()
				
			# import torchviz
			# dot = torchviz.make_dot(model(blocks, batch_inputs), params=dict(model.named_parameters()))
			# dot.render(filename='attached.dot')
			# dot.view()
...
def main():
	...
	# argparser.add_argument('--fan-out', type=str, default='3')
