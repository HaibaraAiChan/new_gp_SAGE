	...
	for run in range(args.num_runs):
		...
		for epoch in range(args.num_epochs):
			...
			for step_out, (input_nodes, seeds, blocks) in enumerate(full_batch_dataloader):
				...
				# sub_sum_loss, model = run_degrees_bucket(unique_degrees_cut, num_split, step, parameters)
				# loss_sum= loss_sum + sub_sum_loss 
				group_sum_loss, model = group_degrees_buckets(unique_degrees_cut, num_split, step, parameters)
				loss_sum= loss_sum + group_sum_loss 
				
				
				# #===----===---===-----=== degree need to split, here we use the last degree 10 as an example
				degree =torch.tensor([unique_degrees[-1].item()], dtype=torch.long) 
				if not torch.equal(degree, torch.tensor([10], dtype=torch.long)):
