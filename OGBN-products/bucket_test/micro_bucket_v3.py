...
def  _bucketing(val, degree, num_split, step): 
	...
	unique_val = (unique(sorted_val)).cpu().detach().numpy()

	bkt_idx = []
	v = degree.item()
	eqidx = nonzero_1d(sorted_val == v) # local nid of degree equals v
	tmp = gather_row(idx, eqidx)
	...
	return unique_val, bkt_idx

def bucketor(data, bkt_idx):
			...
			for step_out, (input_nodes, seeds, blocks) in enumerate(full_batch_dataloader):
				...
				if (degree_split.dim() == 0) and (num_split >= 1):
					...
					for step in range(num_split):
						...
						# degree_split = degree_split.to(torch.int32).to(device)
						# print("blocks[-1].dstnodes: ", blocks[-1].dstdata)
						# print('blocks[-1].dstnodes()[NID] ', blocks[-1].dstdata['_ID'])
						degs = blocks[-1].in_degrees()
						bucket_outputs_local = idx[ sorted_val == degree_split].int().to(device)

						unique_degs, bkt_idx = _bucketing(degs, degree=degree_split, num_split=num_split,step=step)
						
						nids_ = bucketor(blocks[-1].dstnodes(),bkt_idx)            #----=-=-=-=-=-=-log
						original_nids_ = bucketor(blocks[-1].dstdata['_ID'],bkt_idx) #----=-=-=-=-=-=-log
						bucket_outputs_local = torch.stack(nids_).to(torch.int32).squeeze()
						bucket_outputs_global = torch.stack(original_nids_).to(torch.int32).squeeze()
						print('v3.py : output nodes local nid: ', bucket_outputs_local[:10])
						print('v3.py : output nodes global nid: ', bucket_outputs_global[:10])
						see_memory_usage("----------------------------------------- before batch_pred = model(blocks, batch_inputs, deg.to(device)) ")
