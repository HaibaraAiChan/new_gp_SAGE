		...
		for epoch in range(args.num_epochs):
			...
			for step, (input_nodes, seeds, blocks) in enumerate(full_batch_dataloader):
				...
				for deg in unique_degrees:
					...
					# bucket_inputs_local = torch.unique(torch.cat((bucket_outputs, eid_bkt_in))).tolist()
					# mini_batch_src_global = list(OrderedDict.fromkeys(eid_bkt_in))
					...
					# loss_sum_.append(bucket_loss_item ) 
